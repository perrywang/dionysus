...
Dionysus.reqres.setHandler('blog:item', function(id){
	var blog = Blog.findOrCreate({id:id}), defer = $.Deferred();
...
Dionysus.reqres.setHandler('blog:list', function(page, size){
	var blogResources = new BlogCollection(), defer = $.Deferred();
	var pageNumber = page? page: 0;

	blogResources.fetch({data:{projection: 'blogsummary', page: pageNumber, size:size}}).then(function(){
