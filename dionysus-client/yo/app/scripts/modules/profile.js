Dionysus.module('Profile', function(Profile, Dionysus, Backbone, Marionette) {
  ...
  var PofileLayoutView = Marionette.LayoutView.extend({
    ...
    editBlog: function(childView, id) {
      ...
      if (id) {
        Dionysus.request("blog:item", id).done(function(blog) {
          region.show(new BlogEditorView({
            model: blog
          }));
        });
      } else {
    ...
    editFeedBack: function(childView, model) {
      if (model) {
        var region = this.getRegion('myContent');
        var view = new ProfileAppointmentFeedbackView({
          model: model
        });
        region.show(view);
      }
    ...
    uploadAvatar: function() {
      this.getRegion('myContent').show(new uploadAvatarView({
        model: this.model
      }));
    },

    changePass: function() {
      this.getRegion('myContent').show(new changePassView({
        model: this.model
      }));
    },
    ...
    showMyAppointments: function() {
      ...
      if (this.model.toJSON().roles[0].name == "ROLE_CONSULTANT") {
        ...
        Dionysus.request("appointments:search:pageable", "findByConsultant", {
          consultant: sessionStorage.getItem('user'),
        }).done(function(appointments){
          region.show(new ProfileConsultantAppointmentView({collection: appointments}));
        });
        ...
        Dionysus.request("appointments:search:pageable", "findByUser", {
          ...
          region.show(new ProfileAppointmentView({
    ...
    showMyCourses: function() {
      ...
      $.when(Dionysus.request('course:profile:me')).done(function(courses) {
        var courseView = new MyCourseView({
          'courses': courses
        });
    ...
    updateInfo: function() {
      this.getRegion('myContent').show(new ProfileView({
        model: this.model
      }));
    }
  });
  ...
  **/
  ...
  var ProfileConsultantAppointmentItemView = Marionette.ItemView.extend({
    template: JST['templates/home/profile/appointmentsConsultantItem'],
    tagName: 'tr',
    onRender: function() {
      var x;
    }
  ...
  var ProfileConsultantAppointmentView = Marionette.CompositeView.extend({
    template: JST["templates/home/profile/appointmentsConsultantList"],
    childView: ProfileConsultantAppointmentItemView,
    tagName: 'table',
  ...
  var ProfileController = Marionette.Controller.extend({
    showProfile: function(id) {
      ...
      $.when(Dionysus.request('user:entity', id)).done(function(user) {
        var layoutView = new PofileLayoutView({
          model: user
        })
    ...
    showInbox: function() {
      ...
      Dionysus.request('user:notifications', "findMyNotification", {
        size: 50
